...
# store credentials to later send our message via smtp (using gmail here but could be others)
  #I think is saved longterm now that it's committed.
...
#parent directory
##Set parent directory to save outside of the GitHub folder, so Git doesn't try to push 5000 files (not exaggeration)
setwd("G:/My Drive/rbeale data")
...
```{r monthly}
#Calculate how long this code runs, so we can optimize it.
start.time<-Sys.time()


#run the monthly code
monthly(parent_directory, input_rivers)

##List the end time and calculate run time.
end.time<-Sys.time()
end.time-start.time
#### gc() code to speed up run time ####
####Okay. gc() not the move for speeding this up. 

#### next attempt at something ####


#### Communications ####
#send a text that the code is done
sendtext %>%
  smtp_send(
    to = "2034700303@vtext.com", 
    from = "ccosgrov@heidelberg.edu",
...
Review of gc() addition:
#Raw, original code: Length of run time for beaver creek data with 3 strata and 3 iterations: 2.81 minutes
#add gc() into the code. New run time: 2.78 minutes
#Let's add another newer river in, to see. Adding West. Run time w/o gc(): 6.13min
#With gc(): 6.13, so maybe gc() isn't going to solve all our problems. Good try, though. I want to run it once with Honey Creek just to make sure, though.
#Honey creek without gc():28.87min
#Honey Creek with gc(): 28.64 mins
